...
bool RobotShape::occupies(const wxPoint &aPoint) const {
  wxPoint cornerPoints[] = {
      getRobot()->getFrontRight(), getRobot()->getFrontLeft(),
      getRobot()->getBackLeft(), getRobot()->getBackRight()};
 ...
 *
 */
void RobotShape::handleEndDrag() { FUNCTRACE_DEVELOP(); }
/**
 *
 */
...
void RobotShape::drawRobot(wxDC &dc) {
  ...
  dc.DrawLine(centre.x, centre.y,
              static_cast<int>(centre.x + std::cos(angle) * 25),
              static_cast<int>(centre.y + std::sin(angle) * 25));

  int textOffsetx =
      static_cast<int>(std::cos(-angle - 0.5 * Utils::PI) * (titleSize.x / 2) +
                       std::sin(-angle - 0.5 * Utils::PI) * (titleSize.y / 2));
  int textOffsety =
      static_cast<int>(std::sin(-angle - 0.5 * Utils::PI) * (titleSize.x / 2) -
                       std::cos(-angle - 0.5 * Utils::PI) * (titleSize.y / 2));
  dc.DrawRotatedText(title, centre.x - textOffsetx, centre.y + textOffsety,
                     (-angle - 0.5 * Utils::PI) / Utils::PI * 180);
}
/**
 *
 */
