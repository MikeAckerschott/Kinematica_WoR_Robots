...
public:
   ...
   *
   */
  Robot(const std::string &aName, const wxPoint &aPosition);
  /**
   *
   */
  virtual ~Robot();
   ...
   *
   */
  void setSize(const wxSize &aSize, bool aNotifyObservers = true);
  /**
   *
   */
  ...
  /**
   *
   * @return true if the robot is communicating, i.e. listens with an active
   * ServerConnection
   */
  ...
  //@{
  /**
   * This function is called by a ServerSesssion whenever a message is received.
   * If the request is handled, any response *must* be set in the Message
   * argument. The message argument is then echoed back to the requester,
   * probably a ClientSession.
   *
   * @see Messaging::RequestHandler::handleRequest( Messaging::Message&
   * aMessage)
   */
  virtual void handleRequest(Messaging::Message &aMessage);
  /**
   * This function is called by a ClientSession whenever a response to a
   * previous request is received.
   *
   * @see Messaging::ResponseHandler::handleResponse( const Messaging::Message&
   * aMessage)
   */
...
private:
  ...
  /**
   *The believed position of the robot calculated by the  kalman filter
   */
  wxPoint beliefPositionKalman;

  /**
  *The believed route of the robot calculated by the  kalman filter
  */
  std::vector<wxPoint> beliefRouteKalman;

  /**
  *The believed orientation of the robot calculated by kalman filter in radians
  */
  double beliefOrientationKalman;

  /**
   *The believed position of the robot calculated by the particle filter
   */
  wxPoint beliefPositionParticle;

  /**
  *The believed route of the robot calculated by the particle filter
  */
  std::vector<wxPoint> beliefRouteParticle;

  /**
  *The believed orientation of the robot calculated by particle filter in radians
  */
  double beliefOrientationParticle;
