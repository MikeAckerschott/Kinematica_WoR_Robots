...
void RobotWorld::populate(int UNUSEDPARAM(aNumberOfWalls)) {
  RobotWorld::getRobotWorld().newRobot(
      "Robot", wxPoint(163, 111), false); // @suppress("Avoid magic numbers")

  static const wxPoint coordinates[] = {wxPoint(0, 0), wxPoint(0, 1024),
                                        wxPoint(1024, 1024), wxPoint(1024, 0)};

  for (int i = 0; i < 4; ++i) {
    RobotWorld::getRobotWorld().newWall(coordinates[i],
                                        coordinates[(i + 1) % 4], false);
  }

  static const wxPoint extraWalls[] = {
      wxPoint(0,  424), wxPoint(285, 287), wxPoint(824, 177),
      wxPoint(1024, 575), wxPoint(260, 744), wxPoint(297, 479),
      wxPoint(617, 761), wxPoint(342, 900),
  };

  for (int i = 0; i <= 6; i += 2) {
    RobotWorld::getRobotWorld().newWall(extraWalls[i], extraWalls[i + 1],
                                        false);
  }

  //
  //		RobotWorld::getRobotWorld().newGoal( "Goal", wxPoint(850,
  // 500),false); // @suppress("Avoid magic numbers")
  ...
  RobotWorld::getRobotWorld().newGoal(
      "Goal", wxPoint(664,837), false); // @suppress("Avoid magic numbers")
